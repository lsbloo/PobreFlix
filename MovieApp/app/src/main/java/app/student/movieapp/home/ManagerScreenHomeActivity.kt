    ...
    companion object {
        ...
        fun changeLayoutListMoviesByScreenDevice(
            ...
            if (height >= 1600 && width >= 900) {
                ...
                val listMoviesFragment = ListMoviesFragment(activity.baseContext,R.layout.fragment_movie_list,R.layout.movie_item)
                return homeActivity.fragmentCalled(listMoviesFragment, HomeActivity.TAG_LIST_MOVIES_FRAGMENT)
            }else if(height <= 1200 && width <= 750) {
                val homeActivity = activity as HomeActivity
                val listMoviesFragment = ListMoviesFragment(activity.baseContext,R.layout.fragment_movie_list_min,R.layout.movie_item_min)
                return homeActivity.fragmentCalled(listMoviesFragment, HomeActivity.TAG_LIST_MOVIES_FRAGMENT)
            }
            return null
        }
